...
public class RabbitMQConfig1 {
    ...
    public static final String DELAY_QUEUED_NAME = "delay.queue.demo.business.queued";
    ...
    public static final String DELAY_QUEUED_ROUTING_KEY = "delay.queue.demo.business.queued.routingkey";
    ...
    public static final String DEAD_LETTER_QUEUEB_ROUTING_KEY = "delay.queue.demo.deadletter.delay_10s.routingkey";
    ...
    public DirectExchange delayExchange(){
        //安装了插件要指定队列的类型,然后开启延迟队列属性,可以绑定到普通队列也可以绑定到死信队列消费
        Map<String, Object> arguments = new HashMap<>();
        arguments.put("x-delayed-type", "direct");
        DirectExchange directExchange = new DirectExchange(DELAY_EXCHANGE_NAME,true,false,arguments);
    ...
    public Queue delayQueueD(){
        return new Queue(DELAY_QUEUED_NAME);
    }
    ...
    // D绑定关系
    @Bean
    public Binding BindingD(@Qualifier("delayQueueD") Queue queue,
                                 @Qualifier("delayExchange") DirectExchange exchange){
        return BindingBuilder.bind(queue).to(exchange).with(DELAY_QUEUED_ROUTING_KEY);
    }
    // 声明死信队列A绑定关系
    @Bean
